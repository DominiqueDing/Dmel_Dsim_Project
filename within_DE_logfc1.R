...
row.names(y$counts)<-gsub(pattern = "IM23",replacement = "BomBc1",x = row.names(y$counts))
row.names(y$counts)<-gsub(pattern = "IM14",replacement = "Dso2",x = row.names(y$counts))
row.names(y$counts)<-gsub(pattern = "IM1",replacement = "BomS1",x = row.names(y$counts))
row.names(y$counts)<-gsub(pattern = "IM2",replacement = "BomS2",x = row.names(y$counts))
row.names(y$counts)<-gsub(pattern = "CG15065",replacement = "BomS5",x = row.names(y$counts))
row.names(y$counts)<-gsub(pattern = "CG16836",replacement = "BomT2",x = row.names(y$counts))
row.names(y$counts)<-gsub(pattern = "IM4",replacement = "Dso1",x = row.names(y$counts))
row.names(y$counts)<-gsub(pattern = "IMPPP",replacement = "BaraA2",x = row.names(y$counts))

keep.logfc1 <- row.names(y$counts)%in%c(genes.sig.in.any.logfc1)

table(keep.logfc1)
# keep
# FALSE  TRUE 
# 3646  9169
y<-y[keep.logfc1, , keep.lib.sizes=FALSE]
y<-calcNormFactors(y)
y$samples
...
ggplot(data = df.result_DE.mel.sim.r)+
